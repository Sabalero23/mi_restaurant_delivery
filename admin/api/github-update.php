...
function generateSystemId($db) {
    try {
        ...
        $query = "INSERT INTO settings (setting_key, setting_value, description) 
                  VALUES ('system_id', ?, 'ID Ãºnico del sistema')";
        $stmt = $db->prepare($query);
...
function verifyLicense($db) {
    try {
        ...
        if ($isValid) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Clave de licencia invÃ¡lida'
            ]);
        }
...
function checkForUpdates($db) {
    try {
        ...
        if (!isLicenseValid($db)) {
            echo json_encode([
                'success' => false,
                'message' => 'Licencia no vÃ¡lida. Por favor, activa tu licencia para continuar.',
                'requires_license' => true
            ]);
            return;
        }
        
        // Obtener Ãºltimo commit del repositorio
        ...
        // Si hay actualizaciones, obtener mÃ¡s detalles
        $commits = [];
...
function getChanges($db) {
    try {
        ...
        // Obtener comparaciÃ³n de commits
...
function createBackup($db) {
    try {
        ...
        // Crear backup de configuraciÃ³n (sin credenciales sensibles)
        $configBackup = [
            'timestamp' => $timestamp,
...
/**
 * Realizar la actualizaciÃ³n del sistema
 */
function performUpdate($db) {
    try {
        // Verificar licencia
        if (!isLicenseValid($db)) {
            throw new Exception('Licencia no vÃ¡lida. Activa tu licencia para continuar.');
        }
        
        // Iniciar transacciÃ³n de log
        $updateId = createUpdateLog($db);
        
        $settings = getGithubSettings($db);
        $currentCommit = getSystemCommit($db);
        
        // 1. Crear backup si estÃ¡ habilitado
        ...
        // 2. Obtener estadÃ­sticas de GitHub ANTES de descargar
        ...
        // Encontrar el directorio extraÃ­do (tiene el formato: nombre-rama)
        $extractedDirs = glob($extractPath . '/*', GLOB_ONLYDIR);
        if (empty($extractedDirs)) {
            throw new Exception('No se encontraron archivos extraÃ­dos');
        }
        
        $sourceDir = $extractedDirs[0];
        $targetDir = realpath('../../');
        
        // Archivos a excluir de la actualizaciÃ³n
        $excludeFiles = [
            'config/config.php',
            'uploads',
            'admin/uploads',
        ...
        // 8. Actualizar log de actualizaciÃ³n
        updateUpdateLog($db, $updateId, 'completed', 'ActualizaciÃ³n completada exitosamente');
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Error durante la actualizaciÃ³n: ' . $e->getMessage()
        ]);
    }
}

/**
 * Obtener detalles de una actualizaciÃ³n especÃ­fica
 */
function getUpdateDetails($db) {
    try {
        ...
        if (empty($updateId)) {
            throw new Exception('ID de actualizaciÃ³n no especificado');
        }
        ...
        if (!$update) {
            throw new Exception('ActualizaciÃ³n no encontrada');
        }
...
/**
 * Revertir actualizaciÃ³n usando backup
 */
function rollbackUpdate($db) {
    try {
        ...
        if (empty($backupPath)) {
            throw new Exception('No se especificÃ³ el backup a restaurar');
        }
...
function generateUniqueSystemId() {
    // Generar ID Ãºnico basado en varios factores del servidor
    $factors = [
...
/**
 * Obtener historial y estadÃ­sticas de migraciones
 */
...
/**
 * Reintentar una migraciÃ³n fallida
 */
function retryMigration($db) {
    try {
        ...
        if (empty($version)) {
            throw new Exception('VersiÃ³n no especificada');
        }
        ...
        if ($result['success']) {
            echo json_encode([
                'success' => true,
                'message' => "MigraciÃ³n {$version} ejecutada exitosamente",
